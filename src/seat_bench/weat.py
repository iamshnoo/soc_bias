...
def p_val_permutation_test(X, Y, A, B, n_samples, cossims, parametric=False):
    ...
    """
    X = np.array(list(X), dtype=np.int_)
    Y = np.array(list(Y), dtype=np.int_)
    A = np.array(list(A), dtype=np.int_)
    B = np.array(list(B), dtype=np.int_)
